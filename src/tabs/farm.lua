...
local tab  = GUI.MakeTab("Farm", "⚙️", page)

GUI.MakeSection(page, "PLOTS")

GUI.MakeToggle(page,
    "Unlock all plots",
    "Auto unlocks every plot",
    "autoUnlockPlots",
    Theme.Accent,
    function()
        task.spawn(function()
            local plots = workspace:WaitForChild("Map"):WaitForChild("Plots")
            while Toggles.autoUnlockPlots do
                for i = 1, 10 do
                    if not Toggles.autoUnlockPlots then break end
...
GUI.MakeToggle(page,
    "Auto upgrade farm",
    "Keeps upgrading your farm",
    "autoUpgradeFarm",
    Color3.fromRGB(251, 146, 60),
    function()
        task.spawn(function()
            while Toggles.autoUpgradeFarm do
...
GUI.MakeToggle(page,
    "Auto sell crates",
    "Sells crates automatically",
    "autoSellCrates",
    Color3.fromRGB(234, 179, 8),
    function()
        task.spawn(function()
            while Toggles.autoSellCrates do
    ...
    function()
        task.spawn(function()
            while Toggles.autoClaimRewards do
                for i = 1, 100 do
                    if not Toggles.autoClaimRewards then break end
